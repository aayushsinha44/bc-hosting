...
def check_function(request, address, function_name):

    if FunctionModel.objects.filter(user_address=address, name=function_name).count() > 0:
        return HttpResponse(json.dumps({"message": "exists"}), content_type='application/json')
    return HttpResponse(json.dumps({"message": "does not exists"}), content_type='application/json')
